...
newHeader.innerText = 'Exercise: Bicep Curls';
...
function onResultsPose(results) {
  ...
  // Check if previous shoulder positions exist
  if (typeof prev_left_shoulder !== 'undefined' && typeof prev_right_shoulder !== 'undefined') {
    let left_dx = left_shoulder[0] - prev_left_shoulder[0];
    let left_dy = left_shoulder[1] - prev_left_shoulder[1];
    let right_dx = right_shoulder[0] - prev_right_shoulder[0];
    let right_dy = right_shoulder[1] - prev_right_shoulder[1];
    
    if (Math.abs(left_dx) > 0.05 || Math.abs(left_dy) > 0.05 || Math.abs(right_dx) > 0.05 || Math.abs(right_dy) > 0.05) {
      shoulder_moving = true;
    }
  }


  // Set current shoulder positions as previous
  prev_left_shoulder = left_shoulder;
  prev_right_shoulder = right_shoulder;

  // Continue with the remaining code
  const right_elbow = [results.poseLandmarks[13].x, results.poseLandmarks[13].y];
  const right_wrist = [results.poseLandmarks[15].x, results.poseLandmarks[15].y];
  const right_hip = [results.poseLandmarks[23].x, results.poseLandmarks[23].y];
  let right_distance = Math.sqrt(Math.pow((right_elbow[0] - right_hip[0]), 2) + Math.pow((right_elbow[1] - right_hip[1]), 2));
  ...
  const left_elbow = [results.poseLandmarks[14].x, results.poseLandmarks[14].y];
  const left_wrist = [results.poseLandmarks[16].x, results.poseLandmarks[16].y];
  const left_hip = [results.poseLandmarks[24].x, results.poseLandmarks[24].y];
  let left_distance = Math.sqrt(Math.pow((left_elbow[0] - left_hip[0]), 2) + Math.pow((left_elbow[1] - left_hip[1]), 2));
...
if (!shoulder_moving) {
    if (left_distance < 0.3 && right_distance < 0.3) {
      if (angle_left > 165 && angle_right > 165) {
        ...
        let text = "Extend your arms fully, making sure your elbows are almost straight.";
        feedbackDisplay.textContent = text;
      }
  
      if (angle_left < 25 && stage == "extended" && angle_right < 25 && stage == "extended") {
        stage = "compressed";
        counter += 1;
      } else if (angle_left > 25 && angle_right > 25 && stage == "extended") {
        let text = "Contract your arms fully, so that the dumbell almost touches your shoulder.";
        feedbackDisplay.textContent = text;
      }
    } else {
      let text = "Form Tip: Your elbows should be tucked in when performing the bicep curl.";
      feedbackDisplay.textContent = text;
    }
  } else {
