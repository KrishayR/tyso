...
newHeader.innerText = 'Exercise: Front Raise';
...
if (!up && (right_elbow[1]*100 <= right_shoulder[1] * 100) && (left_elbow[1] * 100 <= left_shoulder[1] * 100)) {
    up = true;
    counter += 1;
    let text = "Cool, now lower the weight to the starting position in a controlled manner.";
    feedbackDisplay.textContent = text;
} else if ((right_elbow[1] > right_shoulder[1]) && (left_elbow[1] > left_shoulder[1])){
    up = false;
    let text = "Raise weight straight ahead with arms extended.";
    feedbackDisplay.textContent = text;
} else if ((right_elbow[1] > right_shoulder[1]) && (left_elbow[1] > left_shoulder[1])){
    let text = "Cool, now lower the weight to the starting position in a controlled manner.";
    feedbackDisplay.textContent = text;
