...
newHeader.innerText = 'Exercise: Squats';
...
function onResultsPose(results) {
  ...
  let angle_right = calculateAngles(right_hip, right_knee, right_ankle);
  ...
  let angle_left = calculateAngles(left_hip, left_knee, left_ankle);

  let left_ankle_check = results.poseLandmarks[27]
  let right_ankle_check = results.poseLandmarks[28]
...
canvasCtx5.restore();
// Curling logic
if (left_ankle_check.visibility > 0.5 | right_ankle_check > 0.5) {
      if (angle_left > 165 && angle_right > 165) {
        stage = "extended";
        let text = "Stand shoulder-width apart, keep back straight, bend knees to 90 degrees.";
        feedbackDisplay.textContent = text;

      } else {
        let text = "Perfect! Slowly return to your normal position and continue.";
        feedbackDisplay.textContent = text;
      }
  
      if (angle_left < 145 && stage == "extended" && angle_right < 145 && stage == "extended") {
        stage = "compressed";
        counter += 1;
      }
    } else {
