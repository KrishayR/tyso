...
const compliments = ["Awesome", "Excellent", "Fantastic", "Great", "Good", "Super", "Nice", "Amazing", "Perfect", "Terrific"]
let random_choice = "Great"
...
newHeader.innerText = 'Exercise: Lateral Raise';
...
// Curling logic
if (!up && (right_elbow[1]*100 <= right_shoulder[1] * 100) && (left_elbow[1] * 100 <= left_shoulder[1] * 100)) {
    up = true;
    counter += 1;
    const random = Math.floor(Math.random() * compliments.length);
    random_choice = compliments[random]
    let text = `${random_choice}! Now move them back to your sides.`;
    feedbackDisplay.textContent = text;
} else if ((right_elbow[1] > right_shoulder[1]) && (left_elbow[1] > left_shoulder[1])){
    up = false;
    let text = "Move the weight out to the side until they reach shoulder height.";
    feedbackDisplay.textContent = text;
} else if ((right_elbow[1] > right_shoulder[1]) && (left_elbow[1] > left_shoulder[1])){
    const random = Math.floor(Math.random() * compliments.length);
    random_choice = compliments[random]
...
if (right_elbow[1]*100 + 10 < left_elbow[1]*100){
  let text = "Form Check: Your left arm is higher than your right.";
    feedbackDisplay.textContent = text;
}
if (left_elbow[1]*100 + 10 < right_elbow[1]*100){
  let text = "Form Check: Your right arm is higher than your left.";
    feedbackDisplay.textContent = text;
}
