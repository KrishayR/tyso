...
newHeader.innerText = 'Exercise: Tricep Extensions';
...
//   const left_ankle = [results.poseLandmarks[28].x, results.poseLandmarks[28].y];
  ...
  let text = "To start, lift the dumbell straight overhead, with your arms parallel.";
...
canvasCtx5.restore();
// Curling logic
if (left_elbow[1] < left_shoulder[1] && right_elbow[1] < right_shoulder[1]) {
      if (angle_left > 130 && angle_right > 130) {
        stage = "extended";
        let text = "Now lower it slowly behind your head until your hands touch your back.";
        feedbackDisplay.textContent = text;
      }
      else {
        let text = "Extend your arms up so they are parallel straight over your head";
        feedbackDisplay.textContent = text;
      }

      if (angle_left < 80 && stage == "extended" && angle_right < 80 && stage == "extended") {
        stage = "compressed";
        counter += 1;
      }
      ...
      else if (left_elbow[1]*10 + 0.75 < right_elbow[1]*10){
